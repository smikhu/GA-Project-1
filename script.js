...
// A function that checks for a match
...
// A shuffle function that randomly distributes the cards
function shuffle() {
    cards.forEach(card => {
...
// A move counter that decrements the count each flip cycle(2 flips)
...
// A function that counts when each card is paired
function matchCounter() {
    matched.innerHTML = matches;
    if (matches == win) {
        setWinState("You've won!")
    }
}
...
// A function that resets the cards back to the cover image
